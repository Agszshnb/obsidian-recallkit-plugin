...
- 短内容单次调用模型。
- 长内容按约 20,000 字符切分，并发 2 个分段分析，再综合生成最终卡片。
- 当前总输入安全上限为 500,000 字符。
...
- MinerU 结果目前只保存 `full.md`，不保存完整 zip、图片资源或 `content_list.json`。
- 长文档分析仍按字符切分，尚未利用 MinerU `content_list.json` 做语义分段。
...
- PDF / MinerU 云端模式：配置 MinerU Token，选择 vault PDF，确认能上传、解析、保存 `full.md` 并进入卡片预览。
